...
- `scripts/` is excluded from `ruff` linting and the CC gate (the gate covers `core/`, `gui/`, and plugin source).
- The CC gate and the i18n gate now live in qgis-plugin-analyzer: `qgis-analyzer analyze --max-cc 10` and the `MISSING_I18N` rule (AST-based). The former `check_cc.py` / `verify_i18n_hygiene.py` scripts were retired (see `docs/plans/upstreaming_qgis_analyzer.md`).
- Thresholds (`CC_THRESHOLD = 10`, `MODULE_SIZE_LIMIT = 400`) are defined once in `sync_metrics.py`. Analyzer versions are recorded in `.agent/memory/agent_metrics.json` → `meta.tools`.
