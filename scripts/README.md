...
| :--- | :--- | :--- |
| `check_cc.py` | Cyclomatic complexity gate (CC ≤ 10; `--threshold N` to override) | pre-push hook, `/refactor-code`, `/release-plugin` |
...
- Thresholds (`CC_THRESHOLD = 10`, `MODULE_SIZE_LIMIT = 400`) are defined once in `sync_metrics.py` and passed to `check_cc.py` via `--threshold`. Analyzer versions are recorded in `.agent/memory/agent_metrics.json` → `meta.tools`.
