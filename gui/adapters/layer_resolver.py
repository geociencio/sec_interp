"""Layer resolution adapter (Extract phase).

Resolves layer references (ID, name, or object) to QgsMapLayer objects using
the QGIS project. Lives in the GUI adapters layer because it depends on
``QgsProject.instance()``; the core layer no longer resolves layers itself.
"""
