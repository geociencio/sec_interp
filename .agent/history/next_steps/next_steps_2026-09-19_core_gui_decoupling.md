# Next Steps - Phase v3.8.0 (Updated 2026-09-19)

## 🔄 Core/GUI Decoupling Refactor 2026-09-19 — branch `refactor/core-gui-decoupling`

- Completed Fases 0–4 of `docs/plans/implementation_plan_architecture_refactor_v3.8.0.md` (20 commits on the feature branch).
- **Done**: architecture allowlist gate (36 → 31 `core/` files), Fase 1 dead-code cleanup, Fase 2 God-Object decoupling (PreviewCache/RenderState/page protocol/DI/event bus), Fase 3 preview consolidation, and 10 Fase 4 increments (LayerNotificationManager + DataFetcher → `gui/adapters/`, `controller.py` QGIS-agnostic, Douglas-Peucker/metrics/azimuth/densify in stdlib).
- **Resume**: `PYTHONPATH=.. uv run python3 -m unittest discover tests` (576 OK).
- **Remaining (Fase 4-6)**:
  - Move the real `LayerResolver` implementation to `gui/adapters/` and migrate its core consumers (`preview_service`, validators, `drillhole_orchestrator`, `controller`).
  - Migrate `create_buffer_geometry`/`calculate_segment_range` (Extract) to `gui/adapters/`; migrate QGIS-coupled services (`structure_service`, `geology_service`, `profile_service`, `drillhole_service`, `export_service`).
  - Fase 5: validation DTO-ification; Fase 6: docs + release.
- **Reference**: `docs/maintenance/session_2026-09-19_core_gui_decoupling.md`
- **⚠️ NOT YET MERGED** to `main`; pre-existing uncommitted changes (`MURALLA.qgs`, `pyproject.toml`, `resources.py`, `uv.lock`) are unrelated to this session and were left untouched.
...
3. (Manual) ~~Publish `v3.7.2` draft + upload ZIP to plugins.qgis.org~~ ✅ DONE — live on portal + latest on GitHub.
