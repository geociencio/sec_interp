# -*- coding: utf-8 -*-
"""
Tests for core/utils.py
"""

import math
import unittest
from unittest.mock import Mock, patch, MagicMock
from tests.base_test import BaseTestCase

from sec_interp.core import utils as scu


class TestStrikeParsing(BaseTestCase):
    """Tests for strike parsing functions."""

    def test_parse_strike_numeric(self):
        """Test parsing numeric strike values."""
        self.assertEqual(scu.parse_strike(0), 0)
        self.assertEqual(scu.parse_strike(90), 90)
        self.assertEqual(scu.parse_strike(180), 180)
        self.assertEqual(scu.parse_strike(270), 270)
        self.assertEqual(scu.parse_strike(360), 0.0)

    def test_parse_strike_string_numeric(self):
        """Test parsing string numeric strike values."""
        self.assertEqual(scu.parse_strike("45"), 45.0)
        self.assertEqual(scu.parse_strike("180"), 180.0)

    def test_parse_strike_quadrant_ne(self):
        """Test parsing N-E quadrant notation."""
        result = scu.parse_strike("N 30 E")
        self.assertEqual(result, 30)

    def test_parse_strike_quadrant_nw(self):
        """Test parsing N-W quadrant notation."""
        result = scu.parse_strike("N 45 W")
        self.assertEqual(result, 315)  # 360 - 45

    def test_parse_strike_quadrant_se(self):
        """Test parsing S-E quadrant notation."""
        result = scu.parse_strike("S 60 E")
        self.assertEqual(result, 120)  # 180 - 60

    def test_parse_strike_quadrant_sw(self):
        """Test parsing S-W quadrant notation."""
        result = scu.parse_strike("S 15 W")
        self.assertEqual(result, 195)  # 180 + 15

    def test_parse_strike_invalid(self):
        """Test parsing invalid strike values."""
        self.assertIsNone(scu.parse_strike(None))
        self.assertIsNone(scu.parse_strike("invalid"))
        self.assertIsNone(scu.parse_strike("ABC"))


class TestDipParsing(BaseTestCase):
    """Tests for dip parsing functions."""

    def test_parse_dip_numeric(self):
        """Test parsing numeric dip values."""
        dip, direction = scu.parse_dip("45")
        self.assertEqual(dip, 45.0)
        self.assertIsNone(direction)

    def test_parse_dip_with_direction(self):
        """Test parsing dip with direction."""
        dip, direction = scu.parse_dip("45 NE")
        self.assertEqual(dip, 45.0)
        self.assertEqual(direction, 45)  # NE = 45 degrees

    def test_parse_dip_cardinal_directions(self):
        """Test all cardinal directions."""
        test_cases = [
            ("30 N", 30.0, 0),
            ("45 NE", 45.0, 45),
            ("60 E", 60.0, 90),
            ("30 SE", 30.0, 135),
            ("45 S", 45.0, 180),
            ("60 SW", 60.0, 225),
            ("30 W", 30.0, 270),
            ("45 NW", 45.0, 315),
        ]
        for dip_str, expected_dip, expected_dir in test_cases:
            dip, direction = scu.parse_dip(dip_str)
            self.assertEqual(dip, expected_dip)
            self.assertEqual(direction, expected_dir)

    def test_parse_dip_invalid(self):
        """Test parsing invalid dip values."""
        self.assertEqual(scu.parse_dip(None), (None, None))
        self.assertEqual(scu.parse_dip("invalid"), (None, None))


class TestCardinalToAzimuth(BaseTestCase):
    """Tests for cardinal direction conversion."""

    def test_cardinal_to_azimuth_all_directions(self):
        """Test all cardinal directions."""
        self.assertEqual(scu.cardinal_to_azimuth("N"), 0)
        self.assertEqual(scu.cardinal_to_azimuth("NE"), 45)
        self.assertEqual(scu.cardinal_to_azimuth("E"), 90)
        self.assertEqual(scu.cardinal_to_azimuth("SE"), 135)
        self.assertEqual(scu.cardinal_to_azimuth("S"), 180)
        self.assertEqual(scu.cardinal_to_azimuth("SW"), 225)
        self.assertEqual(scu.cardinal_to_azimuth("W"), 270)
        self.assertEqual(scu.cardinal_to_azimuth("NW"), 315)

    def test_cardinal_to_azimuth_invalid(self):
        """Test invalid cardinal direction."""
        self.assertIsNone(scu.cardinal_to_azimuth("INVALID"))
        self.assertIsNone(scu.cardinal_to_azimuth(""))


class TestApparentDip(BaseTestCase):
    """Tests for apparent dip calculation."""

    def test_calculate_apparent_dip_perpendicular(self):
        """Test when section is perpendicular to strike."""
        # Strike = 90, Dip = 45, Section = 0 (perpendicular)
        result = scu.calculate_apparent_dip(90, 45, 0)
        # Should be close to true dip
        self.assertLess(abs(result - 45), 1.0)

    def test_calculate_apparent_dip_parallel(self):
        """Test when section is parallel to strike."""
        # Strike = 90, Dip = 45, Section = 90 (parallel)
        result = scu.calculate_apparent_dip(90, 45, 90)
        # Should be close to 0
        self.assertLess(abs(result), 1.0)

    def test_calculate_apparent_dip_45_degrees(self):
        """Test at 45 degree angle."""
        result = scu.calculate_apparent_dip(90, 45, 45)
        # Should be between 0 and 45
        self.assertTrue(0 <= abs(result) <= 45)


class TestInterpolation(BaseTestCase):
    """Tests for elevation interpolation."""

    def test_interpolate_elevation_exact_point(self):
        """Test interpolation at exact data point."""
        result = scu.interpolate_elevation(self.sample_profile_data, 100.0)
        self.assertEqual(result, 150.0)

    def test_interpolate_elevation_midpoint(self):
        """Test interpolation at midpoint."""
        result = scu.interpolate_elevation(self.sample_profile_data, 50.0)
        # Should be average of 100 and 150
        self.assertEqual(result, 125.0)

    def test_interpolate_elevation_beyond_range(self):
        """Test interpolation beyond data range."""
        result = scu.interpolate_elevation(self.sample_profile_data, 500.0)
        # Should return last elevation
        self.assertEqual(result, 140.0)

    def test_interpolate_elevation_empty_data(self):
        """Test interpolation with empty data."""
        result = scu.interpolate_elevation([], 100.0)
        self.assertEqual(result, 0)


class TestBufferGeometry(BaseTestCase):
    """Tests for create_buffer_geometry using native algorithm."""

    def test_create_buffer_geometry_basic(self):
        """Test basic buffer creation using native buffer method."""
        from unittest.mock import Mock

        # Create mock geometry
        mock_geom = Mock()
        mock_geom.isNull.return_value = False

        # Mock the buffer method
        mock_buffer_geom = Mock()
        mock_geom.buffer.return_value = mock_buffer_geom

        # Create mock CRS
        mock_crs = Mock()

        # Call function
        result = scu.create_buffer_geometry(mock_geom, mock_crs, 100.0)

        # Verify buffer was called
        mock_geom.buffer.assert_called_once_with(100.0, 5)
        self.assertEqual(result, mock_buffer_geom)

    def test_create_buffer_geometry_invalid_input(self):
        """Test buffer creation with invalid geometry."""
        from qgis.core import QgsGeometry, QgsCoordinateReferenceSystem

        mock_geom = Mock()
        mock_geom.isNull.return_value = True

        mock_crs = Mock()

        # Should raise ValueError
        with self.assertRaisesRegex(ValueError, "Geometry is null or invalid"):
            scu.create_buffer_geometry(mock_geom, mock_crs, 100.0)

    def test_create_buffer_geometry_null_result(self):
        """Test buffer creation returning null geometry."""
        from unittest.mock import Mock

        mock_geom = Mock()
        mock_geom.isNull.return_value = False
        mock_geom.buffer.return_value = None

        mock_crs = Mock()
        result = scu.create_buffer_geometry(mock_geom, mock_crs, 100.0)
        self.assertIsNone(result)


class TestSpatialFiltering(BaseTestCase):
    """Tests for filter_features_by_buffer using spatial index."""

    def test_filter_features_by_buffer_basic(self):
        """Test spatial filtering using native QgsFeatureRequest."""
        from unittest.mock import Mock, MagicMock

        mock_layer = Mock()
        mock_layer.isValid.return_value = True
        mock_layer.crs.return_value = Mock()

        mock_buffer = MagicMock()
        mock_buffer.isNull.return_value = False

        # Mock feature and its geometry
        mock_feat = Mock()
        mock_feat_geom = Mock()
        mock_feat.hasGeometry.return_value = True
        mock_feat.geometry.return_value = mock_feat_geom
        mock_feat_geom.intersects.return_value = True

        mock_layer.getFeatures.return_value = [mock_feat]

        result = scu.filter_features_by_buffer(mock_layer, mock_buffer)

        self.assertEqual(len(result), 1)
        self.assertEqual(result[0], mock_feat)
        mock_layer.getFeatures.assert_called_once()

    def test_filter_features_invalid_layer(self):
        """Test with invalid layer."""
        from qgis.core import QgsVectorLayer, QgsGeometry, QgsCoordinateReferenceSystem
        from unittest.mock import Mock

        mock_layer = Mock()
        mock_layer.isValid.return_value = False

        mock_geom = Mock()
        mock_crs = Mock()

        with self.assertRaisesRegex(ValueError, "Invalid features layer"):
            scu.filter_features_by_buffer(mock_layer, mock_geom, mock_crs)

    def test_filter_features_invalid_buffer(self):
        """Test with invalid buffer geometry."""
        from qgis.core import QgsVectorLayer, QgsGeometry, QgsCoordinateReferenceSystem
        from unittest.mock import Mock

        mock_layer = Mock()
        mock_layer.isValid.return_value = True

        mock_geom = Mock()
        mock_geom.isNull.return_value = True

        mock_crs = Mock()

        with self.assertRaisesRegex(ValueError, "Invalid buffer geometry"):
            scu.filter_features_by_buffer(mock_layer, mock_geom, mock_crs)
