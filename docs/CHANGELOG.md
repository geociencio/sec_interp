...
### Fixed
- **Settings Persistence**: Replaced a silent `try/except/pass` in `_get_setting` with a logged `logger.warning`, so configuration restoration failures are visible instead of hidden (Bandit `B110`).
