# SecInterp — Plugin Directory Structure

> Generated tree of the **plugin-owned** files (`core/`, `gui/`, `exporters/`, `plugin/`,
> `resources/`, `i18n/` and the root entry points). It intentionally excludes development
> and tooling directories (`tests/`, `scripts/`, `docs/`, `examples/`, `help/`, `.agent/`)
...
| Path | Role |
|------|------|
| `sec_interp_plugin.py` | Plugin entry point (facade composing the `plugin/` mixins) |
| `logger_config.py` | Centralized logging + QGIS handler |
| `core/` | Business logic (QGIS-agnostic, thread-safe) |
| `gui/` | UI layer (Extract / Present phases, managers, renderers, tools) |
| `exporters/` | Export strategies behind `BaseExporter` (SHP/GPKG/DXF/CSV/PDF/SVG/…) |
| `plugin/` | Plugin lifecycle/input/render mixins |
| `resources/` | Icons, styles and compiled `resources.py` |
| `i18n/` | Translation sources (`.ts`) and compiled catalogs (`.qm`) |

---

*Generated for the SecInterp Code Walkthrough — keep in sync when the tree changes.*
