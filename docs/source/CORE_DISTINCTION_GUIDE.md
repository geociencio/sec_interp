...
3.  **Result**: Core returns DTOs (Data Transfer Objects) defined in `core/domain/` (`dtos.py`, `entities.py`, `task_inputs.py`). The GUI layer handles converting these back to QGIS layers if needed.
