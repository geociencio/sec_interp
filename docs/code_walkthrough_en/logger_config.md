...
> Logging is initialized **first**, before anything else (`setup_logging()` in `SecInterp.__init__`). See [[sec_interp_plugin]].
...
- [[Index]] — vault index
- [[sec_interp_plugin]] — where `setup_logging()` is called
- [[__init__]] — entry point preceding initialization
- [[safe_loader]] — another fault-tolerance pattern
