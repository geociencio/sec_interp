...
> ```
> See [[controller]] for the 4-level `try` in `_get_and_validate_inputs`.
...
- [[Index]] — vault index
- [[controller]] — where these exceptions are raised and propagated
- [[domain]] — `PreviewParams.validate()` (origin of `ValidationError`)
- [[validation]] — validators raising `ValidationError` / `ParameterError`
- [[sec_interp_plugin]] — final handling in the UI
