...
> See [[01 - sec_interp_plugin]]: `QTranslator.load(SecInterp_<locale>.qm)` in `SecInterp.__init__`.
...
- [[01 - sec_interp_plugin]] — `SecInterp(TranslatableMixin)` and `.qm` loading
- [[10 - controller]] — `ProfileController(TranslatableMixin)` uses `self.tr()` in exceptions
- [[12 - exceptions]] — translatable messages carried in exceptions

---

*Note 18 of the SecInterp Code Walkthrough vault — v3.8.0*
