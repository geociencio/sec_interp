...
> Este archivo de 507 líneas se descompuso en el paquete `plugin/` ([[plugin_mixins]]); `SecInterp` es ahora una **fachada de 129 líneas** que compone los mixins.

**Ruta**: `sec_interp_plugin.py` (129 líneas; antes 507)
...
> - Ver [[safe_loader]] para el detalle del helper.
...
> Desconectar señales antes de destruir objetos es **crítico** en PyQt. Ver [[sec_interp_plugin#🔐 Gestión de señales]] más abajo.
...
`TranslatableMixin` (en [[i18n]]) define:
```python
...
- [[safe_loader]] — `SafeLoader` (DI tolerante)
...
- [[adapters]] — adapters de la fase Extract
